...
summary: A股个股技术分析（均线/头肩底/缠论/筹码/基本面），输出图表与文字报告
user-invocable: true
priority: high
category: finance
triggers:
  ...
  intents:
    ...
    - 做缠论/筹码/均线综合技术分析
not_for:
  - 仅基本面/财务/估值分析 → 用 stock-fundamental-analysis
  - 港股/美股数据 → 用 westockdata
examples:
  - 帮我分析一下贵州茅台的技术面
  - 上证指数缠论分析
---
...
金融技术分析五大模块 —— 均线 + 头肩底 + 缠论 + 筹码 + 基本面。图表输出：缠论图、头肩底颈线图、筹码图、信息图。
...
分析只做五大模块：均线、头肩底、缠论、筹码、基本面。超出范围的指标不要自行补充或编造。
...
| `days` | int | `60` | 回溯交易日天数 |
| `full-history` | flag | — | 拉取从上市日至今的全历史数据 |
...
JSON 数据输出后，**必须**按以下结构生成中文文字分析报告，五大模块缺一不可：
...
### 📌 数据来源
- 行情/K线数据：WeStock（腾讯自选股）优先，akshare fallback
- 筹码分布(CYQ)：WeStock 优先，本地三角分布模型 fallback
- 基本面财务数据：国信证券 API（需 GS_API_KEY）
```

**关键要求**：
1. 五大模块**必须全部输出**，不得省略任何模块
2. 头肩底 detected=false 时也必须输出"未检测到"的说明
3. 所有内容使用中文
4. 禁止输出 KDJ/RSI/DMI/SAR/布林带等不在五大模块内的指标
...
| 信息图 | `infographic_` | 竖版1080x1920，五大模块卡片+操作建议 |
...
├── SKILL.md              # 本文件
├── generate_analysis.py  # 主脚本（一站式：数据+五大模块分析+出图+上传）
