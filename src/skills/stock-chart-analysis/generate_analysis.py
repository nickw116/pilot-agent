...
def normalize_symbol(symbol: str):
    ...
    纯 6 位代码按 A 股代码段判市场与是否指数：
      沪市个股: 60xxxx / 68xxxx(科创板) / 11xxxx(转债) / 13xxxx(转债) / 50xxxx(基金) / 56xxxx
      深市个股: 000xxx/001/002/003 主板中小板, 300/301 创业板, 123/127/128 转债, 15/16 基金
      沪市指数: 000xxx(上证系列, 如 000001=上证指数)
      深市指数: 399xxx
    """
    s = symbol.strip().lower()
    # 兼容 000001.SH / 000001.SZ 形式
    if "." in s:
        parts = s.split(".")
    ...
    if s.startswith("sh"):
        code = s[2:]
        is_idx = code.startswith("0") or code.startswith("999")
        return code, "sh", is_idx
    if s.startswith("sz"):
        code = s[2:]
        is_idx = code.startswith("399") or code.startswith("899")
        return code, "sz", is_idx
    # 纯代码：按 A 股代码段判定
    code = s
    if code.startswith(("60", "68", "11", "13", "50", "56", "58", "900")):
        return code, "sh", False
    if code.startswith("399") or code.startswith("899"):
        return code, "sz", True
    if code.startswith("0"):
        # 000001 等存在二义性：沪市指数 vs 深市个股。约定带前缀(sh/sz)才视为指数。
        # 纯数字 000xxx 默认按深市个股处理。
...
def fetch_data(symbol: str, days: int, full_history: bool = False):
    ...
    if df is None or df.empty:
        ...
        if is_index:
            try:
                df = ak.stock_zh_index_daily_tx(symbol=full_symbol)
                if df is not None and not df.empty:
                    start_fmt = datetime.strptime(start_str, "%Y%m%d")
                    df["date"] = pd.to_datetime(df["date"])
                    df = df[(df["date"] >= start_fmt) & (df["date"] <= end_date)]
                    print(f"[fetch] stock_zh_index_daily_tx: {len(df)} rows")
            except Exception as e:
                errors.append(f"stock_zh_index_daily_tx: {e}")

        if df is None or df.empty:
            try:
                df = ak.stock_zh_a_daily(symbol=full_symbol, start_date=start_str, end_date=end_str, adjust="qfq")
                if df is not None and not df.empty:
                    print(f"[fetch] stock_zh_a_daily: {len(df)} rows")
            except Exception as e:
